        ...
        try:
            ...
            for p in positions: # 依次为：平安证券-银河证券
                ...
                if stock_code == stock_code_new:
                    ...
                    cost_price = p.get('参考成本价') or p.get('cost_price') or 0.0
                    current_price = p.get('当前价') or p.get('current_price') or 0.0
                    market_value = p.get('最新市值') or p.get('market_value') or 0.0
                    total_pl_amount = p.get('浮动盈亏') or p.get('total_pl_amount') or 0.0
                    total_pl_ratio = p.get('盈亏比例(%)') or p.get('total_pl_ratio') or 0.0
                    daily_pl_amount = p.get('当日盈亏') or p.get('daily_pl_amount') or 0.0
                    daily_pl_ratio = p.get('当日盈亏比(%)') or p.get('daily_pl_ratio') or 0.0
                    position_ratio = p.get('仓位占比(%)') or p.get('position_ratio') or 0.0
