...
class ClientTrader(IClientTrader):
    ...
    def _get_balance_from_statics(self):
        ...
        for key, control_id in self._config.BALANCE_CONTROL_ID_GROUP.items():
            try:
                val = self._main.child_window(
                    control_id=control_id, class_name="Static"
                ).window_text()
                result[key] = float(val)
            except (findwindows.ElementNotFoundError, ValueError, IndexError):
                result[key] = 0.0
                print(f"Warning: Control {control_id} not found.")
        return result

    def _get_balance_from_statics_bak(self):
        result = {}
        for key, control_id in self._config.BALANCE_CONTROL_ID_GROUP.items():
            result[key] = float(
